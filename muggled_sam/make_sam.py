...
def make_sam_from_state_dict(
    ...
    """
    Function used to load a SAM v2 or v3 model from a state dict (e.g. model weights).
...
def import_model_functions(state_dict: dict[str, Tensor]) -> Callable | None:
    """
    Helper used to figure out which model type (e.g. v2/v3) we're working with,
    ...
    Returns:
        make_sam_function
        -> Returns None if the model type (e.g. SAM v2/v3) cannot be determined
    """
    ...
    if make_sam_func is None:
        ...
        if mugsamv3p1_target_key in sd_keys:
            from .v3p1_sam.make_sam_v3p1 import make_samv3p1_from_muggled_state_dict as make_sam_func
        elif mugsamv3_target_key in sd_keys:
