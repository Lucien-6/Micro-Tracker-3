...
def save_tracking_label_tif_sequence(frames_dict: dict[int, ndarray], save_folder: str) -> tuple[str, int]:
    """
    ...
    os.makedirs(save_folder, exist_ok=True)
    num_saved = 0
    for seq_idx, frame_idx in enumerate(sorted(frames_dict.keys()), start=1):
