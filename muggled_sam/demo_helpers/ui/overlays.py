...
class MiddleClickCaptureOverlay(BaseOverlay):
    """Captures middle-clicks on the video area for object picking (no visible overlay)."""

    def __init__(self):
        super().__init__()
        self._pick_xy_norm = None
        self._is_changed = False

    def read(self) -> tuple[bool, tuple[float, float] | None]:
        """Returns: is_changed, xy_norm (or None)"""
        is_changed = self._is_changed
        self._is_changed = False
        pick_xy = self._pick_xy_norm
        self._pick_xy_norm = None
        return is_changed, pick_xy

    def on_middle_click(self, cbxy, cbflags) -> None:
        if cbxy.is_in_region:
            self._pick_xy_norm = cbxy.xy_norm
...
class HoverOverlay(BaseOverlay):
    ...
    def __init__(self, read_right_clicks: bool = True):
        super().__init__()
    ...
    def clear(self, flag_is_changed=True, keep_position=False):
        """Reset overlay state"""
    ...
    def read(self) -> tuple[bool, bool, bool, CBEventXY]:
        """Returns: is_changed, is_left_clicked, is_right_clicked, event_xy"""
        is_changed = self._is_changed
        self._is_changed = False
        is_left_clicked = self._is_left_clicked
        is_right_clicked = self._is_right_clicked
        self._is_left_clicked = False
        self._is_right_clicked = False
        return is_changed, is_left_clicked, is_right_clicked, self._event_xy

    def is_in_region(self) -> bool:
        return self._event_xy.is_in_region
