...
class ImmediateButton(BaseCallback):
    ...
    def set_label(self, label: str):
        """Update button label and minimum render width."""

        self._label = f" {label} "
        _, txt_h, _ = self._txtdraw.get_text_size(self._label)
        if txt_h > self._rdr.limits.min_h:
            new_scale = self._txtdraw._scale * (self._rdr.limits.min_h / txt_h) * 0.8
            self._txtdraw.style(scale=new_scale)
        btn_w, _, _ = self._txtdraw.get_text_size(self._label)
        self._rdr.limits.update(min_w=btn_w)
        self.set_debug_name(f"{label.strip()} (ImmediateBtn)")
        return self
