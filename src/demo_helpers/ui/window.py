...
class DisplayWindow:
    ...
    def replace_mouse_callbacks(self, *callbacks):
        """Replace all mouse callbacks (used when rebuilding the display layout)."""
        self._mouse_cbs = CallbackSequencer(*callbacks)
        cv2.setMouseCallback(self.title, self._mouse_cbs)
        self._using_mouse_cbs = True
        return self
    ...
    def attach_arrow_keypress_callback(self, direction: str, callback):
        """
        Attach a callback for an arrow key, registering all known platform key codes.

        Direction must be one of: left, up, right, down
        """
        canonical = _ARROW_DIRECTION_TO_KEY[direction.lower()]
        for keycode in _ARROW_KEY_ALIASES[canonical]:
            self._keypress_callbacks_dict[keycode] = callback
        return self
    ...
    def refocus(self):
        """Try to return keyboard focus to the OpenCV window (helps arrow keys on Windows/Qt)."""
        try:
            cv2.setWindowProperty(self.title, cv2.WND_PROP_TOPMOST, 1)
            cv2.setWindowProperty(self.title, cv2.WND_PROP_TOPMOST, 0)
        except cv2.error:
            pass
        return self
    ...
    def show(self, image, frame_delay_ms=None) -> tuple[bool, int]:
        ...
        raw_keypress = read_waitkey(int(frame_delay_ms))
        keypress = normalize_waitkey(raw_keypress)
        ...
        if self._enable_keypress_callbacks:
            ...
            for cb_keycode, cb in self._keypress_callbacks_dict.items():
                if keypress == cb_keycode or raw_keypress == cb_keycode:
                    cb()
                    self.refocus()
                    break
...
class KEY:
    ...
    # Linux / macOS (low-byte waitKey codes)
    LEFT_ARROW = 81
    UP_ARROW = 82
    RIGHT_ARROW = 83
    DOWN_ARROW = 84

    # Windows OpenCV extended waitKey / waitKeyEx codes
    WIN_LEFT_ARROW = 2424832
    WIN_UP_ARROW = 2490368
    WIN_RIGHT_ARROW = 2555904
    WIN_DOWN_ARROW = 2621440

    # Linux GTK waitKeyEx codes
    GTK_LEFT_ARROW = 65361
    GTK_UP_ARROW = 65362
    GTK_RIGHT_ARROW = 65363
...
_ARROW_KEY_ALIASES = {
    KEY.LEFT_ARROW: (KEY.LEFT_ARROW, KEY.WIN_LEFT_ARROW, KEY.GTK_LEFT_ARROW),
    KEY.UP_ARROW: (KEY.UP_ARROW, KEY.WIN_UP_ARROW, KEY.GTK_UP_ARROW),
    KEY.RIGHT_ARROW: (KEY.RIGHT_ARROW, KEY.WIN_RIGHT_ARROW, KEY.GTK_RIGHT_ARROW),
...
def read_waitkey(delay_ms: int) -> int:
    """
    Read a keypress from OpenCV.

    On Windows, waitKey() often returns 0 for arrow keys; waitKeyEx() is required.
    """
...
def normalize_waitkey(raw_keypress: int) -> int:
    """
    Normalize raw OpenCV waitKey results to stable callback key codes.

    Arrow keys use low-byte codes on Linux/macOS, but extended codes on Windows/GTK.
    """
    ...
    extended_arrow_map = {
        ...
        KEY.WIN_DOWN_ARROW: KEY.DOWN_ARROW,
        KEY.GTK_LEFT_ARROW: KEY.LEFT_ARROW,
